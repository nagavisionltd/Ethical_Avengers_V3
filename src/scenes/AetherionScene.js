class AetherionScene extends BaseScene {
    constructor() {
        super('AetherionScene');
    }

    create(data) {
        this.charKey = data.char || 'cherry';
        const levelWidth = 1920;
        const levelHeight = 1080;

        // --- Setup ---
        this.physics.world.setBounds(0, 0, levelWidth, levelHeight);
        this.add.image(0, 0, 'aetherion_bg').setOrigin(0).setDisplaySize(levelWidth, levelHeight);

        // --- Tilemap with Smart Tiling ---
        const layout = [];
        const levelGridWidth = 60;
        const levelGridHeight = 34;
        ...
        makeSmartPlatform(2, 32, 10);
        makeSmartPlatform(15, 28, 8);
        makeSmartPlatform(30, 25, 6);
        makeSmartPlatform(45, 22, 8);
        makeSmartPlatform(30, 18, 5);
        makeSmartPlatform(15, 14, 7);
        makeSmartPlatform(2, 10, 8);

        const map = this.make.tilemap({ data: layout, tileWidth: 32, tileHeight: 32 });
        const tiles = map.addTilesetImage('aetherion_tileset');
        const groundLayer = map.createLayer(0, tiles, 0, 0);
        groundLayer.setCollisionByExclusion([-1]);

        // --- Player ---
        this.initPlayer(100, 950, levelWidth, this.charKey);
        this.physics.add.collider(this.player, groundLayer);
        this.cameras.main.setBounds(0, 0, levelWidth, levelHeight);

        // --- Enemies ---
        this.spawnDrone(800, 700);
        this.spawnDrone(1200, 500);
        this.spawnDrone(1000, 200);

        // --- Return Portal ---
        const returnPortal = this.add.zone(150, 220 - 100, 100, 400);
        ...
        this.sound.play('music_aetherion', { loop: true, volume: 0.5 });
    }
